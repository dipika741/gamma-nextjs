"use client"; // Optional if you're adding interactivity later

import React from "react";
import ProductAvailableCard from "./ProductAvailableCard";
import { laboratoryProducts, productsData } from "@/data/products"; // Static product data
import slugify from "@/utils/slugify";

const ProductSection = ({ title }) => {
  const targetCategorySlug = "laboratory-equipments";

  const filteredProducts = productsData.filter(
    (product) => slugify(product.category) === targetCategorySlug
  );
